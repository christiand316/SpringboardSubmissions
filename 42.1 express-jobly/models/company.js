...
class Company {
  ...
  static async findAllByOptions(searchConfig) {
    ...
    if (searchConfig.minEmployees) {
      WHERE_COMMANDS.push(`num_employees >= ${searchConfig.minEmployees}`)
    }
    if (searchConfig.maxEmployees) {
      WHERE_COMMANDS.push(`num_employees <= ${searchConfig.maxEmployees}`)
    }
    if (searchConfig.nameLike) {
      const sanitizedName = this.sanitizeString(searchConfig.nameLike)
      WHERE_COMMANDS.push(`name ILIKE '${sanitizedName}%'`)
    }
